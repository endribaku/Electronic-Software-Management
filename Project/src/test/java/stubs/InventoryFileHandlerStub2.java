package stubs;

import Interfaces.DAO.IInventoryFileHandler;
import Models.*;
import javafx.beans.property.ObjectProperty;
import javafx.beans.property.SimpleObjectProperty;
import javafx.collections.FXCollections;
import javafx.collections.ObservableList;

import java.time.LocalDate;

public class InventoryFileHandlerStub2 implements IInventoryFileHandler {

    // ==== What our tests assert ====
    public Supplier lastAddedSupplier;
    public Supplier lastDeletedSupplier;
    public String lastUpdatedSupplierId;
    public String lastUpdatedSupplierName;

    private final ObservableList<Supplier> suppliers = FXCollections.observableArrayList();
    private final ObservableList<Item> suppliedItems = FXCollections.observableArrayList();

    // Interface requires this
    private final ObjectProperty<Inventory> inventory =
            new SimpleObjectProperty<>(new Inventory());

    // ===== SuppliersController needs these =====

    @Override
    public ObservableList<Supplier> getSuppliersList() {
        return suppliers;
    }

    @Override
    public ObservableList<Item> getSuppliedItems() {
        return suppliedItems;
    }

    @Override
    public void addSupplier(Supplier supplier) {
        lastAddedSupplier = supplier;
        suppliers.add(supplier);
    }

    @Override
    public boolean updateSupplier(String supplierID, String supplierName) {
        lastUpdatedSupplierId = supplierID;
        lastUpdatedSupplierName = supplierName;

        // update in-memory list if present
        for (Supplier s : suppliers) {
            if (s.getSupplierID().equals(supplierID)) {
                s.setName(supplierName);
                return true;
            }
        }
        return true;
    }

    @Override
    public void deleteSupplier(Supplier supplier) {
        lastDeletedSupplier = supplier;
        suppliers.remove(supplier);
    }

    // ===== Everything else: required by interface, but unused here =====

    @Override
    public ObjectProperty<Inventory> getInventory() {
        return inventory;
    }

    @Override
    public boolean updateInventory(Inventory inventory) {
        this.inventory.set(inventory);
        return true;
    }

    @Override
    public void updateInventoryFile() { }

    @Override
    public void addSector(Sector newSector) { }

    @Override
    public void addCategory(String sectorName, Category newCategory) { }

    @Override
    public void addItem(Category targetCategory, Supplier targetSupplier, Item newItem) { }

    @Override
    public boolean updateItem(String itemID, String itemName, String itemCategory, String itemSupplier,
                              LocalDate date, double pPrice, double sPrice, int quantity) {
        return true;
    }

    @Override
    public boolean deleteItem(Item item) {
        return true;
    }

    @Override
    public boolean updateCategory(Category category, String categoryName, String sectorName) {
        return true;
    }

    @Override
    public boolean updateSector(Sector sector, String sectorName) {
        return true;
    }

    @Override
    public ObservableList<Item> checkForLowStock(ObservableList<Item> items) {
        return FXCollections.observableArrayList();
    }

    @Override
    public ObservableList<Item> getItemsList() {
        return FXCollections.observableArrayList();
    }

    @Override
    public ObservableList<Item> getItemsOfUser(User user) {
        return FXCollections.observableArrayList();
    }

    @Override
    public ObservableList<Category> getCategoriesOfUser(User user) {
        return FXCollections.observableArrayList();
    }

    @Override
    public ObservableList<Sector> getSectorsOfUser(User user) {
        return FXCollections.observableArrayList();
    }

    @Override
    public ObservableList<Item> getItems() {
        return FXCollections.observableArrayList();
    }

    @Override
    public void insertItem(Item item) { }

    @Override
    public void deleteItem(String itemID) { }
}
